...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onAuthCheckRequested(AuthCheckRequested event, Emitter<AuthState> emit) async {
    try {
      ...
      if (session != null) emit(AuthAuthenticated(user: session.user));
  ...
  Future<void> _onAuthSignInRequested(AuthSignInRequested event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      if (response.user != null) emit(AuthAuthenticated(user: response.user!));
      else emit(const AuthError(message: 'Échec de la connexion'));
    } catch (e) {
      emit(AuthError(message: e.toString()));
    }
  }
  ...
  Future<void> _onAuthSignUpRequested(AuthSignUpRequested event, Emitter<AuthState> emit) async {
    emit(AuthLoading());
    try {
      final response = await _supabase.auth.signUp(email: event.email, password: event.password);
      if (response.user != null) emit(AuthAuthenticated(user: response.user!));
      else emit(const AuthError(message: 'Échec de l\'inscription'));
    } catch (e) {
      emit(AuthError(message: e.toString()));
    }
  }
